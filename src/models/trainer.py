...
def run_training_pipeline() -> dict[str, Any]:
    ...
    joblib.dump(reference_stats, output_dir / "reference_stats.joblib")
    logger.info("Saved reference stats for drift detection")
    ...
    try:
        ...
        logger.info(
            "Calibration — Brier uncalibrated: %.4f, calibrated: %.4f",
            uncalibrated_brier, calibrated_brier,
        )
        if calibrated_brier < uncalibrated_brier:
            best_model = calibrated_model
            joblib.dump(best_model, output_dir / "best_model.joblib")
            logger.info("Calibrated model improved Brier score — using calibrated version")
        else:
            logger.info("Calibrated model did not improve Brier score — keeping uncalibrated")
    except Exception as exc:
        logger.warning("Calibration failed: %s — continuing with uncalibrated model", exc)
    ...
    from src.models.explainer import generate_shap_summary
    try:
