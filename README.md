...
**Clinical Feature Engineering.** Raw electronic health record data including ICD-9 diagnosis codes, medication records, and prior utilization history is transformed into 30+ clinically meaningful features. Diagnosis codes are grouped into 9 clinical categories, medication change patterns are quantified, clinical intensity ratios capture per-day treatment burden, HbA1c and diabetes medication flags encode disease management status, and prior visit frequency is decomposed into utilization risk indicators.
...
**Production Ready Serving.** The best model is served through a FastAPI REST API with Pydantic input validation, risk tier classification (Low / Medium / High), per-prediction SHAP explanations, Prometheus metrics export, and drift detection. The entire stack is containerized with Docker Compose and tested through GitHub Actions CI on every commit.
...
│  │  /model/info           │    │  Model Performance Tab       │ │
│  │  /drift/check          │    │  Diagnostic Plots            │ │
│  │  /metrics (Prometheus) │    │  Per-Patient Explanation     │ │
│  │  Pydantic Validation   │    │                              │ │
│  └────────────────────────┘    └──────────────────────────────┘ │
│                                                                  │
├──────────────────────────────────────────────────────────────────┤
...
│                                                                  │
│  ┌────────────────┐  ┌────────────────┐  ┌───────────────────┐  │
│  │ PSI Drift      │  │ KS Test Drift  │  │ Prometheus        │  │
│  │ Detection      │  │ Detection      │  │ Metrics Export    │  │
│  └────────────────┘  └────────────────┘  └───────────────────┘  │
│                                                                  │
...
### Clinical Intensity Ratios

| Feature | Description |
|---|---|
| `lab_procedure_intensity` | Lab procedures per day in hospital |
| `procedure_intensity` | Procedures per day in hospital |
| `medication_intensity` | Medications per day in hospital |
| `diagnosis_complexity` | Diagnoses per day in hospital |
| `med_change_ratio` | Medication changes relative to active medications |

### Visit Decomposition

| Feature | Description |
|---|---|
| `num_emergency_visits` | Prior emergency room visits |
| `num_inpatient_visits` | Prior inpatient admissions |
| `num_outpatient_visits` | Prior outpatient visits |

### Clinical Flags

| Feature | Description |
|---|---|
| `A1Cresult_abnormal` | Binary flag: 1 if HbA1c result is above 7% |
| `diabetesMed_flag` | Binary flag: 1 if patient is on diabetes medication |
| `change_flag` | Binary flag: 1 if medications were changed during encounter |

### Disposition Grouping

| Feature | Description |
|---|---|
| `discharge_disposition_grouped` | Grouped discharge destination (Home, Transfer, SNF/Rehab, Home Health, AMA, Other) |
| `admission_source_grouped` | Grouped admission source (Referral, Transfer, Emergency, Court/Law, Other) |
...
**Numeric features (23):** age_ordinal, time_in_hospital, num_lab_procedures, num_procedures, num_medications, number_diagnoses, num_med_changes, num_meds_active, total_prior_visits, insulin_changed, high_utilizer, medical_specialty_missing, num_emergency_visits, num_inpatient_visits, num_outpatient_visits, lab_procedure_intensity, procedure_intensity, medication_intensity, diagnosis_complexity, med_change_ratio, A1Cresult_abnormal, diabetesMed_flag, change_flag

**Categorical features (8):** race, gender, diag1_category, diag2_category, diag3_category, admission_type_id, discharge_disposition_grouped, admission_source_grouped
...
```json
{
  ...
  "model_version": "xgboost-v1.0-optuna",
  "threshold": 0.18,
  "n_features": 65,
  "feature_names": ["num__age_ordinal", "..."],
  "training_metrics": {"roc_auc": 0.74, "pr_auc": 0.39, "recall": 0.79},
...
**Request Body:** Array of patient records to compare against training data distribution.
...
```json
{
  ...
  "features": [
    {"feature": "age_ordinal", "psi_score": 0.01, "ks_statistic": 0.05, "ks_p_value": 0.82, "drifted": false}
  ]
}
```
...
Returns Prometheus-formatted metrics including `medrisk_predictions_total`, `medrisk_prediction_latency_seconds`, and `medrisk_drift_detected`.
...
│   ├── models/
│   │   ├── trainer.py                 # Training loop, CV ranking, Optuna, calibration, artifact outputs
│   │   ├── evaluator.py              # Metrics, threshold tuning, ROC/PR curves, calibration
│   │   └── explainer.py              # SHAP per-patient and global summary plot generation
│   ├── monitoring/
│   │   └── drift.py                   # PSI and KS-test drift detection
│   ├── serving/
│   │   ├── api.py                     # FastAPI application with drift and Prometheus endpoints
...
### Expanded Feature Engineering (30+ Features)

The feature set has been significantly expanded from 18 to 31 engineered features:

- **Clinical Intensity Ratios:** `lab_procedure_intensity`, `procedure_intensity`, `medication_intensity`, `diagnosis_complexity`, `med_change_ratio` capture per-day clinical burden and treatment intensity
- **Visit Type Decomposition:** `num_emergency_visits`, `num_inpatient_visits`, `num_outpatient_visits` preserve granular prior utilization patterns
- **Clinical Flags:** `A1Cresult_abnormal` (HbA1c > 7%), `diabetesMed_flag`, `change_flag` capture diabetes management indicators
- **Disposition Grouping:** `discharge_disposition_grouped` and `admission_source_grouped` reduce high-cardinality codes into clinically meaningful groups (Home, Transfer, SNF/Rehab, Emergency, Referral)

### Production Monitoring

- **Drift Detection API:** `POST /drift/check` endpoint runs PSI (Population Stability Index) and Kolmogorov-Smirnov tests against training data distributions, returning per-feature drift reports
- **Model Metadata:** `GET /model/info` returns model type, version, threshold, feature list, training metrics, and quality gate status
- **Prometheus Metrics:** `GET /metrics` exposes `medrisk_predictions_total` (by risk tier), `medrisk_prediction_latency_seconds`, and `medrisk_drift_detected` for integration with Grafana or similar monitoring stacks

### Model Calibration

- **Platt Scaling:** Post-hoc probability calibration using `CalibratedClassifierCV` with sigmoid method, automatically applied when it improves Brier score on the test set
- **Calibration Comparison:** Training pipeline logs both calibrated and uncalibrated Brier scores to MLflow for transparency

### Enhanced Explainability

- **Global SHAP Summaries:** Beeswarm and bar plots are generated during training and saved to `reports/figures/` for population-level feature importance analysis
- **ROC and PR Curves:** Automatically generated during evaluation and saved alongside existing confusion matrix and calibration plots

### Dashboard Improvements

- **Model Performance Tab:** New tab in the Streamlit dashboard displaying training metrics, model metadata, quality gate status, and all diagnostic plots (SHAP, ROC, PR, calibration, confusion matrix)
- **Complete Feature Input:** Sidebar includes all 31 features including v2 additions (visit decomposition, clinical flags, disposition grouping)

### Notebooks

- `notebooks/01_eda.ipynb` — Exploratory data analysis with class imbalance visualization, missing value analysis, and demographic breakdowns
- `notebooks/02_feature_engineering.ipynb` — Step-by-step feature engineering walkthrough with before/after distributions
- `notebooks/03_model_experiments.ipynb` — Multi-model comparison with ROC/PR curves, threshold analysis, and SHAP global summaries
...
- **A/B Testing Framework:** Support for comparing model versions in production with controlled rollouts
